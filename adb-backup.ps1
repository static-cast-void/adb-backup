...
function Make-Structure{
    param(
        [string]$FromPath, # Android linux-style path
        [string]$ToPath, # Windows-style path
        [string[]]$excludePath
    )
    $counter = 0
    ...
    while([bool][int64]$queue.Count){
        ...
        [string]$relativeWinStyle = $relative.Replace('/','\')
        [string]$windowsPath = ("$ToPath\$relativeWinStyle").TrimEnd('\','/')
        Make-Dir "$windowsPath"
        Info-Host "[$counter]","成功创造结构:","$windowsPath" DarkYellow,Cyan,Yellow
...
function Start-Pull{
    ...
    while([bool][int64]$queue.Count){
        ...
        foreach($i in $fileList){
            $counter++
            Make-File "$windowsPath\$i"
            $output = ((pull "$now/$i" "$windowsPath\$i") 2>&1 )
            Info-Host "[$counter]","成功拉取文件:","$windowsPath\$i" DarkYellow,Cyan,Yellow
...
function main {
    ...
    $timer = Start-Timer
    $dirCount = (Make-Structure /storage/emulated/0 $PSScriptRoot -excludePath $excludePath) | Select-Object -Last 1
    ...
    $timer = Start-Timer
    $fileCount = (Start-Pull /storage/emulated/0 $PSScriptRoot -excludePath $excludePath) | Select-Object -Last 1
    $timer = Stop-Timer $timer
